job "dre-node-dev" {
  ...
  group "dre-dev-group" {
    ...
    volume "redis-dre-dev" {
      type = "host"
      read_only = false
      source = "redis-dre-dev"
    }
    ...
    task "psql-dre-dev-task" {
        ...
        vault {
            policies = ["dre-node-dev"]
        }

        template {
            data = <<EOH
            {{with secret "kv/dre-node/dev"}}
...
jemalloc-bg-thread yes
    ...
    task "dre-node-dev-listener" {
      ...
      template {
        ...
        {{end}}
        EVALUATION_WHITELIST_SOURCES="[\"[[ consulKey "smart-contracts/dev/relay-registry-source" ]]\",\"[[ consulKey "smart-contracts/dev/distribution-source" ]]\"]"
        EOH
        destination = "secrets/file.env"
        env         = true
      }
      ...
      service {
        ...
        tags = [
          ...
          "traefik.http.middlewares.corsheader-dre-node-dev.headers.accesscontrolallowmethods=GET,OPTIONS,PUT,POST,DELETE",
          "traefik.http.middlewares.corsheader-dre-node-dev.headers.accesscontrolallowheaders=content-type",
        ...
        check {
          name     = "dre-node-dev health check"
          type     = "http"
          path     = "/alive"
          interval = "5s"
          timeout  = "10s"
